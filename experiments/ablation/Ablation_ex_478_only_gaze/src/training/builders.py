...
def build_clip_splits(cfg: dict, save_root: Path, log):
    ...
    if bool(cfg["data"].get("use_fixed_items_manifest", False)):
        ...
        test_variants = cfg["data"].get("test_variants", ["clean", "masked"])
        ...
        test_eval_clips = {}
        if "clean" in test_variants:
            test_eval_clips["test_clean"] = test_clean_clips
        if "masked" in test_variants:
            test_eval_clips["test_masked"] = test_masked_clips

        split_clip_pairs = [("train", train_clips), ("val", val_clips)] + list(test_eval_clips.items())
        for name, split_clips in split_clip_pairs:
        ...
        for name, split_clips in test_eval_clips.items():
            save_clip_manifest(split_clips, save_root / f"clip_manifest_{name}.json")
...
def _make_preload_common(cfg: dict, log):
    face_cfg = cfg["face"]
    return dict(
        window_size=cfg["window"]["size"],
        window_stride=cfg["window"]["stride"],
...
def _make_loader(cfg: dict, items: list, device: str, *, shuffle: bool, drop_last: bool = False):
    pin = device != "cpu"
    return DataLoader(
        MemoryMultitaskDataset(items),
...
def build_train_val_loaders(cfg: dict, train_clips: list, val_clips: list, device: str, log):
    """Preload only train/val windows. Test windows are intentionally delayed."""
    common = _make_preload_common(cfg, log)
    train_items = preload_multitask_windows(train_clips, desc="preload train", **common)
    val_items = preload_multitask_windows(val_clips, desc="preload val", **common)
    _log_window_variant_counts("train", train_items, log)
    _log_window_variant_counts("val", val_items, log)
    train_loader = _make_loader(cfg, train_items, device, shuffle=True, drop_last=False)
    val_loader = _make_loader(cfg, val_items, device, shuffle=False)
    return train_loader, val_loader, train_items, val_items


def build_eval_loader(cfg: dict, split_name: str, clips: list, device: str, log):
    """Preload one evaluation split and return its DataLoader and items."""
    common = _make_preload_common(cfg, log)
    items = preload_multitask_windows(clips, desc=f"preload {split_name}", **common)
    _log_window_variant_counts(split_name, items, log)
    loader = _make_loader(cfg, items, device, shuffle=False)
    return loader, items
