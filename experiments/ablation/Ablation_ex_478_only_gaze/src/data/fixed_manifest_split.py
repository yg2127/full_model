...
def build_manifest_split_videos(
    *,
    ...
    test_clean_videos = convert_split("test", ["clean"]) if "clean" in test_variants else []
    test_masked_videos = convert_split("test", ["masked"]) if "masked" in test_variants else []

    if not train_videos:
        raise RuntimeError("[fixed manifest] train_videos is empty")
    if not val_videos:
        raise RuntimeError("[fixed manifest] val_videos is empty")
    if "clean" in test_variants and not test_clean_videos:
        raise RuntimeError("[fixed manifest] test_clean_videos is empty")
    if "masked" in test_variants and not test_masked_videos:
