...
try:
    from baselines.dmd_original.dmd_original_multitask import DMDOriginalMultitaskClassifier
except ModuleNotFoundError:
    from dmd_original_multitask import DMDOriginalMultitaskClassifier


DEFAULT_CLASSIFICATION_ROOT = "/data/shared/scuppy/Classification_model_V1"
DEFAULT_CONFIG = str(THIS_DIR / "configs" / "dmd_original_seed42_gaze045_light.yaml")
...
def build_fixed_split_clips(cfg: dict, clips: list, shared: dict, log):
    ...
    split_json_path = Path(fixed_cfg.get("json_path") or cfg.get("paths", {}).get("fixed_items_json"))
...
def main(config_path: str):
    ...
    log = shared["get_logger"]("dmd_original_train", log_file=save_root / "train.log")
    ...
    if fixed_split_cfg.get("enabled", False):
        ...
        shared["save_json"](split_info, save_root / "fixed_manifest_split_info.json")
    else:
    ...
    if masked_drop:
        shared["save_json"](masked_drop, save_root / "test_clean_vs_masked_drop.json")
        drop_rows = []
        for head, metrics in masked_drop.items():
            row = {"head": head}
            for metric, values in metrics.items():
                row[f"clean_{metric}"] = values.get("clean")
                row[f"masked_{metric}"] = values.get("masked")
                row[f"drop_abs_{metric}"] = values.get("drop_abs")
                row[f"drop_rel_{metric}"] = values.get("drop_rel")
            drop_rows.append(row)
        pd.DataFrame(drop_rows).to_csv(save_root / "test_clean_vs_masked_drop.csv", index=False, encoding="utf-8-sig")
