#!/usr/bin/env python3
"""Eval-only prediction export for DMD-original baseline.

Purpose:
  - Load an already trained DMD-original checkpoint (best.pt/last.pt)
...
def main():
    ...
    ap.add_argument("--dmd-root", type=Path, default=Path.cwd(), help="Directory containing dmd_original_multitask.py and train_dmd_original_multitask.py")
    ...
    dmd_root = add_path(args.dmd_root)
    run_dir = args.run_dir.resolve()
    run_dir.mkdir(parents=True, exist_ok=True)

    import train_dmd_original_multitask as td
    from dmd_original_multitask import DMDOriginalMultitaskClassifier

    cfg_path = args.config or first_existing([run_dir / "config.json", dmd_root / "experiments" / "dmd_original_fixed_clean_masked_augmentation_baseline_seed42_20260525_225535.yaml"])
    if cfg_path is None:
    ...
    log = shared["get_logger"]("dmd_original_export_predictions", log_file=run_dir / "export_predictions.log")
