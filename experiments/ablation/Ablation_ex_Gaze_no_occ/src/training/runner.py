...
from src.training.builders import build_clip_splits, build_loaders, build_model, build_train_objects
...
def train_and_evaluate(config_path: str) -> dict:
    ...
    best_epoch = -1
    no_improve = 0
    test_window_msg = " ".join(f"{name}={len(items)}" for name, items in test_eval_items.items())

    notify(
        cfg,
        f"start fusion={cfg['model'].get('fusion', {}).get('kind', 'concat')} "
        f"windows train={len(train_items)} val={len(val_items)} {test_window_msg}",
    )
    ...
    test_outputs = {
        ...
        for name, loader in test_eval_loaders.items()
    }
    ...
    summary = {
        ...
        "n_test_windows_by_split": {name: len(items) for name, items in test_eval_items.items()},
