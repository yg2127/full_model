...
DEFAULT_ROOT = Path('/data/shared/scuppy/hyi/Compare/Compare_Spatiotemporal')
DEFAULT_SEEDS = [42, 43, 44]

# Single new comparator. It uses the same DMS fixed clean/masked protocol but no OCC/reliability input.
DEFAULT_EXPERIMENTS = [
    {
        'name': 'spatiotemporal_decoupling_face',
        'fusion_kind': 'spatiotemporal_decoupling_face',
        'occ_enabled': False,
    },
]
...
def main() -> None:
    ...
    ap.add_argument('--base-config', default='configs/spatiotemporal_dms_base.yaml')
    ...
    generated_dir = root / 'configs' / 'generated_spatiotemporal_dms'
    log_dir = root / 'logs' / 'spatiotemporal_dms'
    plan = []
    ...
    print('=' * 80)
    print('[SDA-TR-inspired DMS comparator seed sweep]')
    print('root        :', root)
