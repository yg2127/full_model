...
DEFAULT_ROOT = Path('/data/shared/scuppy/hyi/Compare/Compare_Pose-guided Multi-task')
DEFAULT_SEEDS = [42, 43, 44]

# Single new comparator. It uses the same DMS fixed clean/masked protocol but no OCC/reliability input.
DEFAULT_EXPERIMENTS = [
    {
        'name': 'pose_guided_token_selection',
        'fusion_kind': 'pose_guided_token_selection',
        'occ_enabled': False,
    },
]
...
def main() -> None:
    ...
    ap.add_argument('--base-config', default='configs/poguise_dms_base.yaml')
    ...
    generated_dir = root / 'configs' / 'generated_poguise_dms'
    log_dir = root / 'logs' / 'poguise_dms'
    plan = []
    ...
    print('=' * 80)
    print('[PO-GUISE-inspired DMS comparator seed sweep]')
    print('root        :', root)
