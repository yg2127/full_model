...
class MultitaskClassifier(nn.Module):
    """Four-head classifier for action/gaze/hands/talk."""
    ...
    def __init__(
        ...
        self.task_fusion_uses_shared = bool(getattr(self.task_fusion, "uses_shared", True))
        self.task_fusion_uses_face = bool(getattr(self.task_fusion, "uses_face", self.task_fusion_uses_shared))
        head_dim = int(self.task_fusion.out_dim)

        self.action_head = nn.Linear(head_dim, num_action)
        self.gaze_head = nn.Linear(head_dim, num_gaze)
        self.hands_head = nn.Linear(head_dim, num_hands)
        self.talk_head = nn.Linear(head_dim, num_talk)
    ...
    def forward(
        ...
        if self.task_fusion_uses_face or self.task_fusion_uses_shared:
            x_face = self.face_region_pool(x_face)
            face_feat = self.face_branch(x_face)
        else:
            # Skeleton-only comparator: keep the same dataloader/protocol, but do
            # not use FaceMesh/OCC evidence in the prediction path.
            face_feat = None

        if self.task_fusion_uses_shared:
        ...
        return {
            "action": self.action_head(task_features["action"]),
            "gaze": self.gaze_head(task_features["gaze"]),
            "hands": self.hands_head(task_features["hands"]),
            "talk": self.talk_head(task_features["talk"]),
        }
