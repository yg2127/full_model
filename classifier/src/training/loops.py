...
IGNORE_LABEL = -100
HEAD_NAMES = ("action", "gaze", "hands", "talk")
...
class MultitaskCriterion(nn.Module):
    ...
    def forward(self, logits: dict[str, torch.Tensor], batch: dict) -> dict:
        ...
        l_action = _safe_ce(self.ce_action, logits["action"], y_action)
        l_gaze_f = _safe_ce(self.ce_gaze_fine, logits["gaze"], y_gaze)
        l_gaze_w = self._gaze_weak_loss(logits["gaze"], y_weak)
        l_hands  = _safe_ce(self.ce_hands, logits["hands"], y_hands)
        l_talk   = _safe_ce(self.ce_talk, logits["talk"], y_talk)

        total = (self.alpha_action * l_action
                 + self.alpha_gaze * (l_gaze_f + self.gaze_weak_weight * l_gaze_w)
                 + self.alpha_hands * l_hands
                 + self.alpha_talk * l_talk)
