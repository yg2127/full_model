#!/usr/bin/env python3
"""occgateRAW: facemesh 좌표계 기준 (정상 facemesh 원본, 가림만 hgnet 변환).
...
def build_one(face_path, occ_labels, dry=False):
    ...
    out_path = Path(str(hg_path).replace("_hgnet478.npz", "_hgnet478_occgateRAW.npz"))
    ...
    zh = np.load(hg_path, allow_pickle=True)              # hgnet (crop112)
    hg = zh["landmarks"].astype(np.float32)
    ...
    for i, (fp, it) in enumerate(samples, 1):
        try:
            ...
            try:
                ...
                op = str(hg).replace("_hgnet478.npz", "_hgnet478_occgateRAW.npz")
